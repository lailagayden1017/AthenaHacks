...
export default function App({ formData }: AppProps) {
  ...
  const [savingsAmount, setSavingsAmount] = useState(formData?.saveAmount ? parseFloat(formData.saveAmount as string) : 500);
  ...
  // Update savingsAmount when formData changes
  useEffect(() => {
    if (formData?.saveAmount) {
      const amount = parseFloat(formData.saveAmount as string);
      if (!isNaN(amount)) {
        setSavingsAmount(amount);
        setEditSavings(amount);
      }
    }
  }, [formData?.saveAmount]);
  ...
  const totalSavings = savingsAmount - grandTotal;
              ...
              <CardContent>
                {isEditingFunds ? (
                  ...
                  <p className="text-2xl font-bold text-green-600">
                    ${(formData?.saveAmount ? parseFloat(formData.saveAmount as string) : savingsAmount).toFixed(2)}
                  </p>
                )}
                <button
                  onClick={() => {
              ...
              <CardHeader className="flex flex-row items-center justify-between">
                ...
                <div className="flex justify-center min-h-[300px] items-center">
                  {showSavingsView ? (
                      <PieChart 
                        spending={grandTotal}
                        savings={savingsAmount - grandTotal < 0 ? 0 : savingsAmount - grandTotal}                        
                        isSavingsView={true} 
                      />
                  ) : (
                      <PieChart
